...
public class ConnectionRepository : IConnectionService
{
	...
	public async Task<ConnectionSettings> UpdateConnectionInformation(ConnectionSettings connectionSettings)
	{
		...
		for (int i = 0; i < lines.Length; i++)
		{
			...
			{
				lines[i] = $"SQL_CONNECTION_STRING_ADRESS= Persist Security Info=False; Server={connectionSettings.Server};Initial Catalog={connectionSettings.Database};User Id={connectionSettings.Username};Password={connectionSettings.Password};TrustServerCertificate=True";
			}
		}
