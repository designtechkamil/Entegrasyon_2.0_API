...
public class WindchillRepository : IWindchillService
{
	...
	public async Task<WrsToken> GetTokenAsync()
	{
		...
		var request = new HttpRequestMessage(HttpMethod.Get, $"http://{_server}/Windchill/servlet/odata/PTC/GetCSRFToken()");
	...
	public async Task<List<WTUsers>> GetFindUserAsync(string? searchTerm)
	{
		...
		var requestUrl = $"http://{_server}/Windchill/servlet/odata/PrincipalMgmt/Users?$select=ID,Name,EMail,FullName&$filter={filterQuery}";
